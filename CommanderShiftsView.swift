...
struct CommanderShiftsView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 18) {
                        FancyHeader(title: "المناوبات الذكية", subtitle: "تخطيط مرن • تحكم كامل")
                        ...
                        LazyVGrid(columns: grid, spacing: 14) {
                            ...
                            ZstackCard(icon: "person.3",
                                       title: "الجنود",
                                       subtitle: "إدارة الأفراد",
                                       tint: Color(hex: 0x34D399)) {
                                SoldiersEditor(soldiers: $soldiers)
                            }
                            ZstackCard(icon: "beach.umbrella",
                                       title: "الإجازات",
                                       subtitle: "طلبات وأيام الراحة",
                                       tint: Color(hex: 0x06B6D4)) {
                                DaysOffEditor(soldiers: soldiers, days: days, daysOff: $daysOff)
                            }
                        }
                        .padding(.horizontal, 16)
                        .padding(.top, 4)

                        // ملخص الجدولة + جاهزية المناوبات
                        VStack(alignment: .leading, spacing: 10) {
                            HStack(spacing: 10) {
                                Image(systemName: "chart.bar.fill").font(.title2.bold()).foregroundColor(.white)
                                Text("ملخص الجدولة").font(.title3.weight(.semibold)).foregroundColor(.white)
                            }
                            if let r = result,
                        ...
                        .padding(16)
                        .background(
                            LinearGradient(colors: [Color(hex: 0x10B981), Color(hex: 0x059669)],
                                           startPoint: .topLeading, endPoint: .bottomTrailing)
                        )
                        ...
                        VStack(spacing: 12) {
                            Button {
                                generate()
                                SMHHaptics.soft()   // ← الاسم الجديد للهبتكس
                            } label: {
                                HStack(spacing: 10) {
                            ...
                            if let r = result {
                                HStack(spacing: 10) {
                                    NavigationLink {
                                        ...
                                        Label("عرض الجدول", systemImage: "list.bullet.rectangle")
                                    ...
                                    Button {
                                        exportPDF()
                                        SMHHaptics.light()  // ← الاسم الجديد للهبتكس
                                    } label: {
            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Capsule()
                        ...
                        .overlay(
                            HStack(spacing: 6) {
                                Image(systemName: "shield.lefthalf.filled").foregroundStyle(Color(hex: 0x22C55E))
                                Text("S.M.H").font(.footnote.weight(.semibold)).foregroundStyle(.primary)
                            }
            ...
            .onAppear {
                ...
                if SimpleDiskStore.exists(StoreFiles.templates) {
                    if let loadedTemplates: [ShiftTemplate] = try? SimpleDiskStore.load([ShiftTemplate].self, from: StoreFiles.templates),
                       !loadedTemplates.isEmpty {
                        templates = loadedTemplates
                    } else if let loaded = try? TemplatesDiskStore.load(), !loaded.isEmpty {
                        templates = loaded
                    } else {
                        templates = DemoData.defaultTemplates
                    }
                } else if let loaded = try? TemplatesDiskStore.load(), !loaded.isEmpty {
                    templates = loaded
                } else {
                ...
                if SimpleDiskStore.exists(StoreFiles.soldiers),
                   let loadedSoldiers: [Soldier] = try? SimpleDiskStore.load([Soldier].self, from: StoreFiles.soldiers),
                   !loadedSoldiers.isEmpty {
                    soldiers = loadedSoldiers
                }

                if SimpleDiskStore.exists(StoreFiles.daysOff),
                   let loadedDaysOffArray: [DayOff] = try? SimpleDiskStore.load([DayOff].self, from: StoreFiles.daysOff) {
                    daysOff.formUnion(Set(loadedDaysOffArray))
                }

                if SimpleDiskStore.exists(StoreFiles.constraints),
                   let loadedConstraints: SchedulingConstraints = try? SimpleDiskStore.load(SchedulingConstraints.self, from: StoreFiles.constraints) {
    ...
    private func generate() {
        ...
        if SimpleDiskStore.exists(StoreFiles.soldiers),
           let loadedSoldiers: [Soldier] = try? SimpleDiskStore.load([Soldier].self, from: StoreFiles.soldiers),
           !loadedSoldiers.isEmpty {
            self.soldiers = loadedSoldiers
        }

        if SimpleDiskStore.exists(StoreFiles.templates),
           let loadedTemplates: [ShiftTemplate] = try? SimpleDiskStore.load([ShiftTemplate].self, from: StoreFiles.templates),
           !loadedTemplates.isEmpty {
        ...
        if SimpleDiskStore.exists(StoreFiles.daysOff),
           let loadedDaysOffArray: [DayOff] = try? SimpleDiskStore.load([DayOff].self, from: StoreFiles.daysOff) {
            self.daysOff.formUnion(Set(loadedDaysOffArray))
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let input = SchedulerEngine.Input(
                ...
                days: self.days,
                templatesPerDay: self.templates,
            ...
            DispatchQueue.main.async {
                ...
                if let m = computeRosterMetrics(result: out,
                    ...
                    NotificationCenter.default.post(
                        name: Notification.Name("rosterReadinessUpdated"),
                        object: nil,
                        userInfo: [
    ...
    private func saveAll() {
        try? SimpleDiskStore.save(soldiers, to: StoreFiles.soldiers)
        try? SimpleDiskStore.save(templates, to: StoreFiles.templates)
        try? SimpleDiskStore.save(Array(daysOff), to: StoreFiles.daysOff)
        try? SimpleDiskStore.save(constraints, to: StoreFiles.constraints)
    }

    private func computeRosterMetrics(result: ScheduleResult,
                                      soldiers: [Soldier],
                                      templates: [ShiftTemplate],
                                      constraints: SchedulingConstraints,
                                      days: [Date])
    -> (nightStd: Double, restPct: Double, hoursPct: Double, conflicts: Int, readinessScore: Double)? {
        ...
        let endDay = cal.date(byAdding: .day, value: 1, to: cal.startOfDay(for: days.last ?? Date()))!
        ...
        for (_, arr) in grouped {
            let sorted = arr.sorted { l, r in
                guard let lt = tplById[l.templateId], let rt = tplById[r.templateId] else { return l.date < r.date }
        ...
        let withinMax = soldiers.filter { (hoursPer[$0.id] ?? 0) <= Double(constraints.maxHoursPerWeek) }.count
        ...
        return (nightStd, restPct.rounded(), hoursPct.rounded(), conflicts, readinessClamped.rounded())
    }
}

struct RosterSummaryView: View {
    let metrics: (nightStd: Double, restPct: Double, hoursPct: Double, conflicts: Int, readinessScore: Double)

    var body: some View {
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                metric(title: "عدالة الليالي", value: String(format: "%.2f", metrics.nightStd), suffix: "std")
                metric(title: "راحة كافية", value: "\(Int(metrics.restPct))", suffix: "%")
            }
            HStack(spacing: 12) {
                metric(title: "ساعات ضمن الحد", value: "\(Int(metrics.hoursPct))", suffix: "%")
                metric(title: "تعارضات", value: "\(metrics.conflicts)", suffix: "")
            }
            CardBackground {
                HStack(spacing: 14) {
                    DonutProgress(progress: metrics.readinessScore/100)
                        .frame(width: 86, height: 86)
                    VStack(alignment: .trailing, spacing: 6) {
                        Text("جاهزية المناوبات").font(.subheadline).foregroundStyle(.white.opacity(0.85))
                        Text("\(Int(metrics.readinessScore))%")
                            .font(.system(size: 28, weight: .heavy))
                            .monospacedDigit()
                            .foregroundStyle(.white)
                        Text("مزيج: عدالة 40% • راحة 30% • ساعات 20% • تعارض 10%")
        ...
        CardBackground {
            VStack(alignment: .trailing, spacing: 6) {
                ...
                HStack(spacing: 6) {
                    ...
                    if !suffix.isEmpty { Text(suffix).font(.caption).foregroundStyle(.white.opacity(0.7)) }
                }
            }
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(title).font(.system(size: 28, weight: .bold, design: .rounded))
                .foregroundColor(.white).shadow(color: .black.opacity(0.45), radius: 10, y: 8)
            Text(subtitle).font(.callout.weight(.medium)).foregroundStyle(.white.opacity(0.75))
        }
...
private struct SimpleDiskStore {
    ...
    static func save<T: Codable>(_ value: T, to filename: String) throws {
...
private enum StoreFiles {
    static let soldiers = "smh_soldiers"
    static let templates = "smh_templates"
    static let daysOff = "smh_daysOff"
    static let constraints = "smh_constraints"
}

#Preview { CommanderShiftsView() }
